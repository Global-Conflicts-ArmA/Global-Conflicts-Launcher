...
const listDiff = (list, basepath, mod, callback) => {
  ...
  recursive(pathf.join(basepath, mod.Directories), (err, files) => {
    ...
    const toDelete = files.filter(val => !modlist.includes(val))
...
const downloadFile = (callback, item, mod, basepath, i) => {
  if (cancelDownload) {
    ...
    const dest = basepath + item.RelativPath
    const folder = dest.replace(item.FileName, '')
...
const downloadFileR = (list, index, basepath, mod, torrent) => {
  ...
  const dest = basepath + cur.RelativPath
  const folder = dest.replace(cur.FileName, '')
